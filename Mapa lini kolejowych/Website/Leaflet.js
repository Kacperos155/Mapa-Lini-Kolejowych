...
const electrifiedLineColor = "rgb(230, 165, 50)";
...
function selectGetStation(ID) {
    let url = main_url + '/getElement/rail_station/' + ID;
    console.log(url);
    fetch(url)
        .then(response => response.json())
        .then((result) => {
...
function selectGetLine(ID) {
    let url = main_url + '/getElement/rail_line/' + ID;
    console.log(url);
    fetch(url)
        .then(response => response.json())
        .then((result) => {
...
function selectLine(result) {
    ...
    selection = L.geoJSON(result, {
        style: { color: "rgb(64, 64, 64)", weight: 15 }
    });
    selection.addTo(map);
